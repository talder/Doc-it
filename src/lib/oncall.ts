/**
 * On-Call report module.
 * Global immutable log stored in SQLite KV (key: oncall.json).
 * Only the solution field may be updated after creation.
 */
...
export interface OnCallEntry {
  ...
  linkedDoc?: { name: string; category: string; spaceSlug: string };
...
const SETTINGS_FILE = "oncall-settings.json";

const DEFAULT_SETTINGS: OnCallSettings = {
  allowedUsers: [],
  emailEnabled: false,
  emailRecipients: [],
  emailSendTime: "08:00",
...
export async function readOnCallData(): Promise<OnCallData> {
  return readJsonConfig<OnCallData>(ONCALL_FILE, { nextNumber: 1, entries: [] });
}

async function writeOnCallData(data: OnCallData): Promise<void> {
  await writeJsonConfig(ONCALL_FILE, data);
}

export async function readOnCallSettings(): Promise<OnCallSettings> {
  return readJsonConfig<OnCallSettings>(SETTINGS_FILE, { ...DEFAULT_SETTINGS });
}

export async function saveOnCallSettings(settings: OnCallSettings): Promise<void> {
  await writeJsonConfig(SETTINGS_FILE, settings);
}

// ── Access Control ─────────────────────────────────────────────────────────────

export async function isOnCallAllowed(username: string, isAdmin: boolean): Promise<boolean> {
  if (isAdmin) return true;
  const settings = await readOnCallSettings();
  return settings.allowedUsers.includes(username);
}
...
export interface CreateOnCallFields {
  registrar: string;
  date: string;
  time: string;
  description: string;
...
export async function addOnCallEntry(fields: CreateOnCallFields): Promise<OnCallEntry> {
  const data = await readOnCallData();
  const num = data.nextNumber || 1;
  const now = new Date().toISOString();
  const entry: OnCallEntry = {
    id: `ONC-${String(num).padStart(6, "0")}`,
    registrar: fields.registrar,
    date: fields.date,
    time: fields.time,
    description: fields.description,
    workingMinutes: fields.workingMinutes,
    assistedBy: fields.assistedBy ?? [],
...
/** Update only the solution field. All other fields are immutable. */
export async function updateOnCallSolution(id: string, solution: string): Promise<OnCallEntry | null> {
  const data = await readOnCallData();
  ...
  const registrarSections = Array.from(byRegistrar.entries())
    ...
    .map(([registrar, regEntries]) => {
      ...
      <tr>
        ...
        <td style="${tdStyle}">${e.linkedDoc ? `<a href="/?space=${encodeURIComponent(e.linkedDoc.spaceSlug)}&doc=${encodeURIComponent(e.linkedDoc.name)}&cat=${encodeURIComponent(e.linkedDoc.category)}" style="color:#1d4ed8;text-decoration:underline">${e.linkedDoc.name}</a>` : "—"}</td>
      </tr>`
        )
        .join("");
