    ...
    <div className="db-filter-panel">
      ...
      {filters.length > 1 && (
        <div className="db-filter-logic">
          <button className={`db-filter-logic-btn${filterLogic === "and" ? " active" : ""}`} onClick={() => onChange(filters, "and")}>AND</button>
          <button className={`db-filter-logic-btn${filterLogic === "or" ? " active" : ""}`} onClick={() => onChange(filters, "or")}>OR</button>
        </div>
      )}
        ...
        {filters.map((f, idx) => {
          ...
          return (
            <div key={idx} className="db-filter-row">
              ...
              <select className="db-filter-select" value={f.op} onChange={(e) => updateFilter(idx, { op: e.target.value as DbFilterOp })}>
                {ops.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
              {needsValue && col?.type === "select" ? (
                <select className="db-filter-select" value={String(f.value || "")} onChange={(e) => updateFilter(idx, { value: e.target.value })}>
                  <option value="">—</option>
              ...
              <button className="db-filter-remove" onClick={() => removeFilter(idx)}><X className="w-3 h-3" /></button>
            </div>
          );
        })}
      </div>
      <button className="db-filter-add" onClick={addFilter}><Plus className="w-3.5 h-3.5" /> Add filter</button>
    </div>
  );
}
